...
class MyFrame(wx.Frame):    
    def __init__(self):
        super().__init__(parent=None, title='Uzbek - English dictionary')
        panel = wx.Panel(self)        
        my_sizer = wx.BoxSizer(wx.VERTICAL)        
        self.text_ctrl = wx.TextCtrl(panel)
        my_sizer.Add(self.text_ctrl, 0, wx.ALL | wx.EXPAND, 5)        
        
        my_btn = wx.Button(panel, label='TRANSLATE')
    ...
    def on_press(self, event):
        ...
        if not value:                       
            self.text_out.SetValue("Please, enter the word")
        else:
